...
public class ControleurConvertisseurArgent 
{
	...
	public void convertirDevise()
	{
		...
		{
			montantConverti = montant.getMontantOriginal() * TestDiagnostique.TAUX_CAD_VERS_USD;
		}
		else if(devise.compareTo("EUR") == 0) // eur == devise
		{
			montantConverti = montant.getMontantOriginal() * TestDiagnostique.TAUX_CAD_VERS_EURO;						
		}
		else if(devise.compareTo("BTC") == 0) // btc == devise
		{
			montantConverti = montant.getMontantOriginal() * TestDiagnostique.TAUX_CAD_VERS_BTC;												
		}
		System.out.println("Le montant converti est " + montantConverti);
		
		montant.setMontantConverti(montantConverti);
	
		// Le controleur demande a la vue d'afficher le traitement
		vue.afficherMontantConverti(this.montant);
	}
